...
import { CreateReturn, Office, Product, Return } from "@/types";
...
export const ReturnManagement: FC<Props> = ({ data }) => {
  ...
  const columns = [
    ...
    columnHelper.accessor("createdAt", {
      header: () => <span>Fecha de Retorno</span>,
  ...
  const {
    ...
    formState: { errors },
    setValue,
  } = useForm<CreateReturn>({
    resolver: yupResolver(schema),
  });

  const mutation = useMutation({
    mutationFn: async (data: CreateReturn) => {
      const response = await createReturn(data);
      reset();
      return response;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["returns"], exact: true });
    },
  });

  const onSubmit: SubmitHandler<CreateReturn> = async (data) => {
    console.log(data);
    setIsDialogOpen(false);
    const promesa = mutation.mutateAsync(data);
    toast.promise(promesa, {
      loading: "Cargando....",
      success: "Retorno cread correctamente",
      error: "Ocurrió un error al crear el retorno",
      duration: 1000,
    });
  };
  ...
  return (
    <div className="p-2">
      <h1 className="mb-2 text-xl font-bold">Administración de Retornos</h1>
      <div className="flex justify-between py-4 font-semibold text-ellipsist-xl">
        ...
        <div>
          <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
            ...
            <DialogContent>
              <DialogHeader>
                <DialogTitle>Crear Nueva Retorno</DialogTitle>
                <DialogDescription>
                  Ingrese los detalles de el nuevo retorno a crear.
                </DialogDescription>
              </DialogHeader>
