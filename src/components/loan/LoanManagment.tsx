...
import { CreateLoan, Loan, Office, Product } from "@/types";
...
export const LoanManagement: FC<Props> = ({ data }) => {
  ...
  const columns = [
    ...
    columnHelper.accessor("createdAt", {
      header: () => <span>Fecha de Prestamo</span>,
  ...
  const mutation = useMutation({
    mutationFn: async (data: CreateLoan) => {
      const response = await createLoan(data);
      reset();
      return response;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["loans"], exact: true });
    },
  });
  ...
  const onSubmit: SubmitHandler<CreateLoan> = async (data) => {
    ...
    toast.promise(promesa, {
      loading: "Cargando....",
      success: "Prestamo creado correctamente",
      error: "Ocurrió un error al crear el prestamo",
      duration: 1000,
    });
  };
  ...
  return (
    <div className="p-2">
      <h1 className="mb-2 text-xl font-bold">Administración de Prestamos</h1>
          ...
          <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
            ...
            <DialogContent>
              <DialogHeader>
                ...
                <DialogDescription>
                  Ingrese los detalles de el nuevo prestamo a crear.
                </DialogDescription>
              </DialogHeader>
