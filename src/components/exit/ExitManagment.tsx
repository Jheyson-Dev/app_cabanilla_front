...
import { CreateExit, Loan, Office, Product } from "@/types";
import { CirclePlus, Search, SlidersHorizontal } from "lucide-react";
import { yupResolver } from "@hookform/resolvers/yup";
import * as yup from "yup";
import { createExit } from "@/service";
import {
  Select,
  SelectContent,
  SelectItem,
...
export const ExitManagement: FC<Props> = ({ data }) => {
  ...
  const mutation = useMutation({
    mutationFn: async (data: CreateExit) => {
      const response = await createExit(data);
      reset();
      return response;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["exits"], exact: true });
    },
  });

  const onSubmit: SubmitHandler<CreateExit> = async (data) => {
    setIsDialogOpen(false);
    const promesa = mutation.mutateAsync(data);
    toast.promise(promesa, {
      loading: "Cargando....",
      success: "Salida creada correctamente",
      error: "Ocurrió un error al crear la salida",
      duration: 1000,
    });
  };
  ...
  return (
    <div className="p-2">
      <h1 className="mb-2 text-xl font-bold">Administración de Salidas</h1>
      <div className="flex justify-between py-4 font-semibold text-ellipsist-xl">
        ...
        <div>
          <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
            ...
            <DialogContent>
              <DialogHeader>
                <DialogTitle>Crear Nueva Salida</DialogTitle>
                <DialogDescription>
                  Ingrese los detalles de la nueva salida a crear.
                </DialogDescription>
              </DialogHeader>
